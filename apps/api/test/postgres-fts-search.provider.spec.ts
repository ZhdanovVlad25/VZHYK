...
function mockStorage() {
  return { getSignedUrl: jest.fn().mockResolvedValue('https://signed.example/photo.jpg') };
}
...
describe('PostgresFtsSearchProvider', () => {
  describe('search', () => {
    it('будує запит з websearch_to_tsquery/ts_rank, коли передано q', async () => {
      ...
      const provider = new PostgresFtsSearchProvider(ds as never, mockStorage() as never);
    ...
    it('резолвить mainMediaUrl через StorageProvider, коли головне фото є', async () => {
      const storage = mockStorage();
      const ds = mockDataSource([row({ mainMediaId: 'm-1', mainMediaStorageKey: 'listings/l-1/x.jpg' })]);
      const provider = new PostgresFtsSearchProvider(ds as never, storage as never);

      const result = await provider.search({});

      expect(storage.getSignedUrl).toHaveBeenCalledWith('listings/l-1/x.jpg');
      expect(result.items[0].mainMediaUrl).toBe('https://signed.example/photo.jpg');
    });

    it('mainMediaUrl=null, коли фото немає', async () => {
      const storage = mockStorage();
      const ds = mockDataSource([row()]);
      const provider = new PostgresFtsSearchProvider(ds as never, storage as never);

      const result = await provider.search({});

      expect(storage.getSignedUrl).not.toHaveBeenCalled();
      expect(result.items[0].mainMediaUrl).toBeNull();
    });
  });
