...
export default function AdminDashboardPage() {
  ...
  if (!authLoading && !user) {
    return (
      <div className="mx-auto max-w-md px-4 py-16 text-center">
        <p className="mb-4 text-gray-700">Щоб побачити дашборд, потрібно увійти.</p>
        <Link href="/login">
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <h1 className="mb-6 text-xl font-semibold text-gray-900">Дашборд</h1>
      ...
      {authLoading || isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <Card>
            <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-500">Користувачі</h2>
          ...
          <Card>
            <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-500">Оголошення</h2>
            <p className="text-2xl font-bold text-gray-900">{metrics.listings.total}</p>
          ...
          <Card>
            <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-500">Модерація</h2>
            ...
            <Link href="/admin/moderation" className="mt-3 inline-block text-sm text-brand-600 hover:underline">
              Перейти до черги →
            </Link>
          </Card>

          <Card>
            <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-500">Скарги</h2>
            <div className="flex flex-wrap gap-2">
              <Badge tone="warning">Нові: {metrics.reports.pending}</Badge>
              <Badge tone="info">В обробці: {metrics.reports.reviewing}</Badge>
            </div>
            <Link href="/admin/reports" className="mt-3 inline-block text-sm text-brand-600 hover:underline">
              Перейти до скарг →
            </Link>
          </Card>

          <Card>
            <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-500">Anti-fraud</h2>
            <p className="text-2xl font-bold text-gray-900">{metrics.riskFlaggedUsers}</p>
            <p className="mt-1 text-sm text-gray-600">користувачів з risk score понад поріг</p>
          </Card>
        </div>
      ) : null}
    </div>
